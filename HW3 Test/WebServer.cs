...
namespace CS422
{
    class WebServer
    {
        ...
        private static WebRequest BuildRequest(TcpClient client)
        {
            ...
            while (x > 0 && y < validReq.Length)
            { //while the ammount read is greater than 0 bytes
                i = 0;
            ...
            while (true)
            {
                ...
                else
                {
                    ...
                    fullRequest += System.Text.Encoding.Default.GetString(streamBuff);
                }

                if (x <= 0)
                    return null; //never had headers. invalid request!
